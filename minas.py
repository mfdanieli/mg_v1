...
def input_data():
    ...
    phosphorus_total = st.sidebar.slider('Total phosphorus', 0.1, 10.0, df['phosphorus_total'].mean())
    BOD = st.sidebar.slider('BOD', 0.1, 10.0, df['BOD'].mean())
    ph = st.sidebar.slider('pH', 5.15, 12.0, df['pH'].mean())
    temperature = st.sidebar.slider('Temperature', 7.5, 31.0, df['temperature'].mean())
    do = st.sidebar.slider('Dissolved oxygen (mg/L)', 2.0, 9.5, df['dissolved_oxygen'].mean())
    turbidity = st.sidebar.slider('Turbidity', 10.0, 1000.0, df['turbidity'].mean())
    conductivity = st.sidebar.slider('Conductivity', 5.0, 400.0, df['condutividade_elétrica_in_loco'].mean())
    suspended_solids = st.sidebar.slider('Suspended solids', 1.0, 1000.0, df['sólidos_em_suspensão_totais'].mean())
    dissolved_solids = st.sidebar.slider('Dissolved solids', 1.0, 1000.0, df['sólidos_dissolvidos_totais'].mean())
